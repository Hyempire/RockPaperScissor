...
def DeleteBackground(roi, center_color, thresH, thresS, thresV, max_area = 0):
    ...
    mask=maskHSV
    cv2.imwrite("D:/GIT/RockPaperScissor/z_PresentationImage/mask1.png", mask)

    # Morph
    morph = cv2.getStructuringElement(cv2.MORPH_RECT, (3,3))
    # Erosion
    mask = cv2.erode(mask, morph)
    cv2.imwrite("D:/GIT/RockPaperScissor/z_PresentationImage/mask2_erosion.png", mask)
    ...
    mask = blank_image
    cv2.imwrite("D:/GIT/RockPaperScissor/z_PresentationImage/mask3_max.png", mask)

    # 테두리 확장
    kernel = np.ones((3,3),np.uint8)
    #extrapolate the hand to fill dark spots within
    mask = cv2.dilate(mask,kernel,iterations = 2)
    cv2.imwrite("D:/GIT/RockPaperScissor/z_PresentationImage/mask4_expand.png", mask)
    
    # 빈틈 메꾸기
    kernel = np.ones((5,5),np.uint8)
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel)
    cv2.imwrite("D:/GIT/RockPaperScissor/z_PresentationImage/mask5_morphology.png", mask)
